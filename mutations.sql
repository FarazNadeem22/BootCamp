...
-- A: We would need a bridge table between employees and categories that has a composit key of employee_id and category_id
...
CREATE TABLE employees_categories(employee_id INT, category_id INT, PRIMARY KEY(employee_id,category_id));
...
ALTER TABLE employees_categories ADD CONSTRAINT fk_ec_employees
FOREIGN KEY (employee_id) REFERENCES employees (employee_id) -- 4.3: Alter table
...
ALTER TABLE employees_categories ADD CONSTRAINT fk_ec_categories
FOREIGN KEY (category_id) REFERENCES categories (category_id) -- 4.4: Insert records
